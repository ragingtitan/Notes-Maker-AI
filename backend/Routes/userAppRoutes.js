...
router.post('/response', async (req, res) => {
    try {
        ...
        const newPrompt = await db.collection(`collection_${decodedToken._id}`).find({}).toArray();
        console.log(newPrompt);
        const newPromptString = JSON.stringify(newPrompt);
        const chat = `${newPromptString}\nYou: ${prompt}\nGEMINI: `;

        const genAI = new GoogleGenerativeAI(process.env.API_KEY);
        const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
        const context = "You are a Notes Maker AI. Your primary task here is to help students excel in their studies. Your job is to multilayered. You will analyze the inputted prompt throughly, present it's clear and detailed summary of atleast 150 words, along with the key conceps, points, key words and potential exam relevant questions along with their answers. You will be open to user feedback and regenerate the unsatisfactory notes. You have currently no output limitations. Just be sure to reject the nsfw contents."
        const result = await model.generateContent(`"context":${context}, "prompt":${chat }`);
    ...
    `
        ...
        res.json({ prompt: prompt, response: text, timestamp: new Date(),  status: true });
    } catch (err) {
